...
public class AkRoomPortal : AkTriggerHandler
{
	...
	private ulong frontRoomID { get { return AkRoom.IsRoomActive(frontRoom) ? frontRoom.GetID() : AkRoom.INVALID_ROOM_ID; } }
	private ulong backRoomID { get { return AkRoom.IsRoomActive(backRoom) ? backRoom.GetID() : AkRoom.INVALID_ROOM_ID; } }
	...
	private void SetRoomPortal()
	{
		...
		{
			UnityEngine.Debug.LogWarning(name + " Portal placement is invalid. The portal is not set in the Spatial Audio engine. The front and back Rooms of the Portal cannot be the same or be part of the same Reverb Zone hierarchy.");
			if (portalSet)
			{
	...
	public bool IsValid
	{
		get
		{
			...
			if (isPortalValid)
			{
				var frontRootID = AkRoom.IsRoomActive(frontRoom) ? frontRoom.GetRootID() : AkRoom.INVALID_ROOM_ID;
				var backRootID = AkRoom.IsRoomActive(backRoom) ? backRoom.GetRootID() : AkRoom.INVALID_ROOM_ID;
				isPortalValid = frontRootID != backRootID;
			}
