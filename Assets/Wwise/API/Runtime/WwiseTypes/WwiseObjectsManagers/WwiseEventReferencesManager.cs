        ...
        public void SetLanguageAndReloadLocalizedBanks(string language, List<string> eventNames = null)
        {
...
#if AK_WWISE_ADDRESSABLES && UNITY_ADDRESSABLES
            var adressableBanks = new List<WwiseAddressableSoundBank>();
#endif
            foreach (var eventReference in m_wwiseEventReferences.Values)
            {
                if (eventNames == null || (eventNames.Count > 0 && eventNames.Contains(eventReference.DisplayName)))
                {
                    ...
                    if (eventReference.AutoBank.CurrentLanguage == "SFX" || eventReference.AutoBank.CurrentLanguage == language)
                        continue;
                    adressableBanks.Add(eventReference.AutoBank);
#endif
            ...
            AkAddressableBankManager.Instance.SetLanguageAndReloadLocalizedBanks(language, adressableBanks);
#else
